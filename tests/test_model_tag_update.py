...
def test_update_tag__valid_tag_id(app, tag_id):
    """Test update() from tag model with valid tag_id."""
...
def test_update_tag__valid_user_id(app, user_id):
    """Test update() from tag model with valid user_id."""
    with app.app_context():
...
def test_update_tag__not_found_tag_id(app, tag_id):
    """Test update() from tag model with tag_id that does not exist."""
    with app.app_context():
...
def test_update_tag__not_found_user_id(app, user_id):
    """Test update() from tag model with user_id that does not exist."""
    with app.app_context():
        with pytest.raises(NotFoundError) as e:
            _tag = Tag(tag_id=1, user_id=user_id, name='tag name')
            result = _tag.update()
...
def test_update_tag__invalid_tag_id(app, tag_id):
    """Test update() from tag model with invalid tag_id."""
    with app.app_context():
...
def test_update_tag__invalid_name(app, tag_name):
    """Test update() from tag model with invalid name."""
    with app.app_context():
        with pytest.raises(InvalidAttributeError) as e:
            _tag = Tag(tag_id=1, user_id=1, name=tag_name)
            result = _tag.update()
            assert result is None
