...
def test_add_score__valid_user_id(app, user_id):
    ...
    with app.app_context():
        _score = Score(
            user_id=user_id, workout_id=2, value="60", rx=True, note="test note"
        )
        score_id = _score.add()
...
def test_add_score__valid_workout_id(app, workout_id):
    ...
    with app.app_context():
        _score = Score(
            user_id=1, workout_id=workout_id, value="60", rx=True, note="test note"
        )
        score_id = _score.add()
        assert score_id is not None
        assert isinstance(score_id, int) is True
...
def test_add_score__not_found_user_id(app, user_id):
    ...
    with app.app_context():
        with pytest.raises(NotFoundError) as e:
            _score = Score(
                user_id=user_id, workout_id=1, value="60", rx=True, note="test note"
            )
            score_id = _score.add()
            assert score_id is None
...
def test_add_score__invalid_value(app, score_value):
    ...
    with app.app_context():
        with pytest.raises(InvalidAttributeError) as e:
            _score = Score(
                user_id=1, workout_id=1, value=score_value, rx=True, note="test note"
            )
            score_id = _score.add()
            assert score_id is None
...
def test_add_score__invalid_rx(app, score_rx):
    ...
    with app.app_context():
        with pytest.raises(InvalidAttributeError) as e:
            _score = Score(
                user_id=1, workout_id=1, value="60", rx=score_rx, note="test note"
            )
            score_id = _score.add()
            assert score_id is None
...
def test_add_score__invalid_note(app, score_note):
    ...
    with app.app_context():
        with pytest.raises(InvalidAttributeError) as e:
            _score = Score(
                user_id=1, workout_id=1, value="60", rx=True, note=score_note
            )
            score_id = _score.add()
            assert score_id is None
